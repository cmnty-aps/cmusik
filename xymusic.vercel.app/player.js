...
function UU(){
    ...
    if(fc)fc.src=S.ct.cover || window.FI;if(ft)ft.innerText=S.ct.title;if(fa)fa.innerText=S.ct.artist;
...
function PK(s,i){
    ...
    S.il=true;
    UB();
    if(S.yp&&S.yr&&S.ct.videoId)S.yp.loadVideoById({videoId:S.ct.videoId});
...
function showToast(msg){var toast=document.createElement('div');toast.className='fixed bottom-24 left-1/2 -translate-x-1/2 btn-chrome font-bold px-5 py-2.5 rounded-full shadow-2xl z-[999]';toast.style.animation='slideUp 0.3s ease-out forwards';toast.innerHTML='<i data-lucide="check" class="w-4 h-4 inline mr-2"></i>'+msg;document.body.appendChild(toast);setTimeout(function(){toast.remove();lucide.createIcons();},2000);lucide.createIcons();}
