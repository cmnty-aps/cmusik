const App={
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Xmusic',cover=FI,artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'Xmusic',cover:FI,artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();
            setTimeout(function(){FullPlayer.open();S.il=true;UB();if(S.yp&&S.yr)S.yp.loadVideoById({videoId:videoId});},400);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Xmusic',cover=FI;
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','Xmusic',FI);});
    },
    ...
    switch(t){
        S.at=t;['home','search','library'].forEach(function(id){gid('view-'+id).style.display='none';});
        if(t==='library'){Library.render();}
        gid('view-'+t).style.display='block';
        ['home','search','library'].forEach(function(n){var b=gid('nav-'+n);if(!b)return;b.classList.remove('text-[#cfd3d8]');b.classList.add('text-[#6b7280]');var i=b.querySelector('i');if(i)i.classList.remove('fill-current');});
        var ab=gid('nav-'+t);if(!ab)return;ab.classList.remove('text-[#6b7280]');ab.classList.add('text-[#cfd3d8]');var icon=ab.querySelector('i')||ab.querySelector('svg');if(t==='home'&&icon)icon.classList.add('fill-current');
